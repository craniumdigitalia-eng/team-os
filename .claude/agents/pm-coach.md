...
6. **Respeite autoridades exclusivas** (pm-qa→veredictos de tarefa, pm-data→schema/CLI).
...
| Memória | Path | Função |
|---|---|---|
| **agent-memory** | `.claude/agent-memory/pm-coach/` | Sua memória PRIVADA — padrões aprendidos, decisões históricas, contexto acumulado entre sessões. Escreva aqui o que ajuda você a trabalhar melhor da próxima vez. |
